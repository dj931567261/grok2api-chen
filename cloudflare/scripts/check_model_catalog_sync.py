...
Checks:
1) app/services/grok/**/model.py and cloudflare/src/grok/models.ts expose the exact same model ids.
2) cloudflare/src/**/*.ts must not contain removed legacy model identifiers.
"""
...
PY_MODEL_CANDIDATES = (
    ROOT / "app" / "services" / "grok" / "services" / "model.py",
    ROOT / "app" / "services" / "grok" / "model.py",
)
PY_MODEL_FILE = next((path for path in PY_MODEL_CANDIDATES if path.exists()), PY_MODEL_CANDIDATES[0])
TS_MODEL_FILE = WORKER_ROOT / "src" / "grok" / "models.ts"
TS_SRC_DIR = WORKER_ROOT / "src"
...
def main() -> int:
    ...
    if py_ids != ts_ids:
        ...
        if only_python:
            lines.append(f"Only in {PY_MODEL_FILE.relative_to(ROOT).as_posix()}:")
            lines.extend(f"  - {mid}" for mid in only_python)
        if only_workers:
            lines.append("Only in cloudflare/src/grok/models.ts:")
            lines.extend(f"  - {mid}" for mid in only_workers)
        errors.append("\n".join(lines))

    removed_hits = _scan_removed_identifiers()
    if removed_hits:
        lines = ["Removed model identifiers found under cloudflare/src/**/*.ts:"]
