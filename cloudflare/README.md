...
> 这个目录是独立的 Cloudflare 子项目。本文档中的命令默认在 `cloudflare/` 目录内执行。
...
- **前端交互与主线保持一致**：Workers 子项目保留独立静态资源，但页面布局与移动端交互和主线版本对齐，包含手机端抽屉导航、表格横向滚动、API Key 居中悬浮新增弹窗等交互
...
>
> 另外：`public/_worker.js` 是 Pages Advanced Mode 的入口文件。Workers 部署时会被 `public/.assetsignore` 排除，避免被当成静态资源上传导致部署失败。
...
```bash
npx wrangler pages deploy public --project-name <你的Pages项目名> --commit-dirty
```
